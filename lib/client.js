		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.7.2：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* composer seat: the shipped seat paints a 36px fade (transparent → bg-base) that in
				   dark themes read as a hard, uncoordinated gradient band — replace it with the
				   same solid bg-base as the conversation card itself: full-width, light/dark
				   consistent, no banding. Messages scrolling behind the composer now hide
				   naturally instead of showing through the seat's sides (set to a translucent
				   color-mix here if you want them faintly visible again) */
				'[data-vsc-layout] [data-composer-seat]{background:var(--dsw-alias-bg-base)!important}'
			].join('\n')
