		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.5.4：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* unified card border system (v1.5.4): between every two adjacent cards there is
				   exactly ONE 1px border-l2 divider (same token as the sidebar's right edge),
				   drawn ON the dock card — right-docked → its left edge, bottom-docked → its top
				   edge. The shipped DetailsPanel's own border-left is removed so right-docked
				   never stacks two lines. The conversation card carries NO junction border: in
				   content-box layout a missing border would leave its content 1px short and
				   expose a transparent strip (v1.5.3 regression). Probe-verified geometry is
				   untouched: no paints, no shadow bands. */
				'[data-vsc-layout] [data-vsc-card="sidebar"]{border-right:1px solid var(--dsw-alias-border-l2)!important}',
				'[data-vsc-layout] [data-vsc-card="dock"]:not([data-vsc-float]):not([data-vsc-bottom]){border-left:1px solid var(--dsw-alias-border-l2)!important}',
