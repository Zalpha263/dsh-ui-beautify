		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.5.5：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* unified card border system (v1.5.5): between every two adjacent cards there is
				   ...
				   border-l3 token. border-l2 single lines (0.12 white in dark / 0.10 black in
				   light) proved invisible on the same-bg-base junction — the sidebar seam only
				   "showed color" because sidebar-fill and bg-base differ; l3 (0.16/0.12) is the
				   shipped strong-divider step and reads clearly on both. The DetailsPanel's own
				   border-left stays removed so right-docked never stacks two lines; the
				   conversation card carries no junction border (no content-box strip risk).
				   Probe-verified geometry is untouched: no paints, no shadow bands. */
