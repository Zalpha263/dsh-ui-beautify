	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SHEET = [
				...
				'[data-vsc-layout] [data-vsc-card="dock"][data-vsc-bottom]{border-left:none!important}',
				/* bottom-docked divider: dsh-style — the column above carries the 1px border on its
				   neighbor edge (like sidebarCol border-right / detailsCol border-left), drawn at the
				   exact row boundary so no hairline can appear between the panels */
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]{border-bottom:1px solid var(--dsw-alias-border-l2)!important}',
