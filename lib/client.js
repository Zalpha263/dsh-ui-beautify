window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function DockHost() {
				useEngine()
				if (resolveFrame() === null || geom.w <= 0 || settingsOpen) return null
				const parts = []
			...
			function watch() {
				...
				ro = new ResizeObserver(function () {
					...
					/* v1.12.13: re-apply — the effective size constraints depend
					   on the CURRENT frame size; without this a window shrink
					   left the fixed tracks stale (grid overflow, handles off
					   the card edges) until the next drag. */
					applyLayout()
				})
				ro.observe(frame)
