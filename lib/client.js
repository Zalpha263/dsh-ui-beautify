	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function Panel() {
				...
				const pill = function (active) {
					return {
						...
						border: active ? "1px solid var(--dsw-static-deepseek-500)" : "1px solid var(--dsw-alias-border-l2)",
						background: active ? "var(--dsw-static-deepseek-500)" : "transparent",
			...
			const SHEET = [
				...
				'[data-vsc-layout] [data-vsc-card="dock"][data-vsc-bottom]{border-left:none!important;border-top:1px solid var(--dsw-alias-border-l2)!important}',
				...
				'[data-vsc-dockbtn]{border:1px solid var(--dsw-alias-border-l2);background:transparent;color:var(--dsw-alias-label-secondary);border-radius:6px;font-size:11px;padding:1px 7px;cursor:pointer;line-height:1.5}',
				'[data-vsc-dockbtn]:hover{color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-border-l3)}',
				'[data-vsc-pop]{position:absolute;display:flex;gap:4px;padding:5px;background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,.18);z-index:33;pointer-events:auto}'
			].join('\n')
			...
			function applyDockedResize(kind, p, startX, startY, base, rect) {
				if (kind === 'sb') {
					...
					setVar('--vsc-rows', 'minmax(0,1fr) ' + h + 'px')
				}
			}
				...
				function up() {
					...
					if (kind === 'sb') {
						if (layoutState.sidebar.width < 140) {
							...
							if (layoutState.sidebar.width < 264) layoutState.sidebar.width = 264
							if (collapsedSidebar && layoutSvc !== undefined) layoutSvc.toggleSidebar()
						}
					}
					if (kind === 'dk-right') {
						if (layoutState.dock.width < 140) {
							...
							if (layoutState.dock.width < 300) layoutState.dock.width = 300
							if (collapsedDetails && layoutSvc !== undefined) layoutSvc.openDetails()
						}
					}
			...
			function dockBtn(label, action, key) {
				return React.createElement('button', {
					key: key, type: 'button', 'data-vsc-dockbtn': '',
			...
			function pushDockedHandles(parts) {
				...
				if (dkR) {
					parts.push(React.createElement('div', {
						...
						onPointerEnter: function () { hoverHandle = 'dk'; notify() },
						onPointerLeave: function () { hoverHandle = null; notify() },
				...
				if (dkB) {
					parts.push(React.createElement('div', {
						...
						onPointerEnter: function () { hoverHandle = 'db'; notify() },
						onPointerLeave: function () { hoverHandle = null; notify() },
			...
			function pushPopover(parts) {
				...
				const dkB = dk.dock === 'bottom' && dVis
				const dkR = dk.dock === 'right' && dVis
				if (!((hoverHandle === 'dk' || hoverHandle === 'db' || popHover) && dVis && (dkR || dkB))) return
				const popLeft = dkR ? w - dk.width + 8 : w - 190
				const popTop = dkR ? 84 : h - dk.height - 38
				parts.push(React.createElement('div', {
					key: 'pop', 'data-vsc-pop': '',
					style: { left: popLeft, top: popTop },
					onPointerEnter: function () { popHover = true; notify() },
					onPointerLeave: function () { popHover = false; notify() },
					children: [
						dockBtn('右侧', function () { setDock('dock', 'right') }, 'r'),
						dockBtn('底部', function () { setDock('dock', 'bottom') }, 'b'),
						dockBtn('浮动', function () { setDock('dock', 'float') }, 'f'),
						dockBtn('关闭', function () { closeDock() }, 'x')
					]
				}))
			}
