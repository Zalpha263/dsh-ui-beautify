	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SHEET = [
				...
				/* bottom-docked junction (v1.4.8): 1) row 1 is a deterministic calc in applyLayout,
				   so the column fills exactly; 2) belt-and-braces: the conversation column gets
				   background-COLOR (longhand — the frame's background-image must stay visible) plus
				   an 8px solid band below it (box-shadow) hiding any residual strip; 3) gap and
				   alignment pinned to 0; 4) the panel's border-top is the divider */
				'[data-vsc-layout][data-vsc-dock-bottom]{gap:0!important;align-content:stretch!important}',
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]:not([data-vsc-float]){background-color:var(--dsw-alias-bg-base)!important;box-shadow:0 8px 0 var(--dsw-alias-bg-base)!important}',
