		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.5.3：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* unified card border system (v1.5.3): between every two adjacent cards there is
				   exactly ONE 1px border-l2 divider, always owned by the conversation card —
				   right-docked → its right edge, bottom-docked → its bottom edge. The dock card
				   no longer draws a junction border (the old single border-top was nearly
				   invisible in dark themes, which read as "no divider"), and the shipped
				   DetailsPanel's own border-left is removed so right-docked never stacks two
				   lines. The sidebar's right edge is unified to border-l2 as well (card mode
				   only). Probe-verified geometry stays untouched: no paints, no shadow bands. */
				'[data-vsc-layout] [data-vsc-card="sidebar"]{border-right:1px solid var(--dsw-alias-border-l2)!important}',
				'[data-vsc-layout][data-vsc-dock-right] [data-vsc-card="conversation"]:not([data-vsc-float]){border-right:1px solid var(--dsw-alias-border-l2)!important}',
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]:not([data-vsc-float]){border-bottom:1px solid var(--dsw-alias-border-l2)!important}',
				'[data-vsc-layout] [data-vsc-card="dock"]:not([data-vsc-float]){border-left:none!important;border-top:none!important}',
				'[data-vsc-layout] [data-vsc-card="dock"] [data-slot="details"] > *{border-left:none!important}',
				...
				/* docked placeholder surface: the hint carries its own bg-base layer. It must
				   NOT be painted on the dock card itself — the shipped DetailsPanel also paints
				   bg-base, and two stacked translucent layers made the dock darker than the
				   conversation card (same dimming as the removed conversation-column paint) */
				'[data-vsc-layout] [data-vsc-card="dock"]:not([data-vsc-float]) > [data-slot="details"]:empty::before{background:var(--dsw-alias-bg-base)}',
			...
			function applyLayout() {
				...
				frame.toggleAttribute('data-vsc-dock-right', dkR)
			...
			function removeAll() {
				...
				frame.removeAttribute('data-vsc-dock-right')
