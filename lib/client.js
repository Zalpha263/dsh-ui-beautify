	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const LAYOUT_DEFAULTS = {
				...
				pluginPanel: { open: false, float: false, width: 400, fx: 140, fy: 80, fw: 560, fh: 420, listHeight: 56 },
				panels: {}
			}
			...
			function mergeLayoutState(saved) {
				...
				if (saved && typeof saved === 'object') {
					...
					if (saved.pluginPanel && typeof saved.pluginPanel === 'object') {
						...
						base.pluginPanel.listHeight = Math.min(260, Math.max(44, base.pluginPanel.listHeight))
					}
			...
			const SHEET = [
				...
				/* v1.9.0: docked cards stay square — the composer card
				   (.uV2eYG_card, native 22px) and the stats panel (.JObwrW_panel,
				   native 12px) are the big surfaces that would otherwise show
				   rounded corners against the docked card edges; floating cards
				   keep their own 12px radius (rule above). Inner micro-elements
				   (bubbles, buttons, popovers) keep their radius. */
				'[data-vsc-layout] [data-vsc-card]:not([data-vsc-float]) [class*="uV2eYG_card"]{border-radius:0!important}',
				'[data-vsc-layout] [data-vsc-card]:not([data-vsc-float]) [class*="JObwrW_panel"]{border-radius:0!important}',
				'[data-vsc-layout] [data-vsc-card]:not([data-vsc-float]) [data-slot="conversation"] > *{border-radius:0!important}',
				/* v1.9.0: 0.2s morph between docked (square, no shadow) and
				   floating (12px, lv3 shadow); instant while dragging. */
				...
				'[data-vsc-layout] [data-vsc-card] :is([class*="uV2eYG_card"],[class*="JObwrW_panel"]){transition:border-radius var(--ds-transition-duration) var(--ds-ease-in-out)!important}',
				'[data-vsc-layout][data-vsc-dragging] [data-vsc-card], [data-vsc-layout][data-vsc-dragging] [data-vsc-card] :is([class*="uV2eYG_card"],[class*="JObwrW_panel"]){transition:none!important}',
				...
				'.ubeautify-ppentry{display:inline-flex;align-items:center;justify-content:center;gap:6px;height:32px;padding:0 10px;border:none;background:transparent;border-radius:6px;color:inherit;cursor:pointer;font-size:14px;line-height:1;flex-shrink:1;transition:background var(--ds-transition-duration) var(--ds-ease-in-out)}',
				'.ubeautify-ppentry:hover{background:var(--dsw-alias-interactive-bg-hover)}',
				'.ubeautify-ppentry.ubeautify-ppon{background:color-mix(in srgb, var(--dsw-alias-brand-primary) 24%, transparent)}',
				...
				/* v1.9.0: accessibility — visible focus ring (2px brand outline
				   + 2px offset) on every plugin-owned control */
				'[data-vsc-layout] [data-vsc-dhost] button:focus-visible,[data-vsc-layout] [data-vsc-pfloat] button:focus-visible,[data-vsc-layout] [data-vsc-ppfloat] button:focus-visible,[data-vsc-layout] [data-vsc-pplist] button:focus-visible,[data-vsc-layout] .ubeautify-ppentry:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:2px}',
				/* v1.9.0: reduced-motion — kill transitions inside plugin-owned
				   containers only (the core UI keeps its own behavior) */
				'@media (prefers-reduced-motion: reduce){[data-vsc-layout] [data-vsc-dhost] *,[data-vsc-layout] [data-vsc-pfloat] *,[data-vsc-layout] [data-vsc-ppfloat] *,[data-vsc-layout] [data-vsc-pplist] *,[data-vsc-layout] .ubeautify-ppentry{transition:none!important}}',
				/* v1.9.2: tab close/float targets — 24px hit areas with hover feedback */
				'[data-vsc-layout] [data-vsc-tabx]{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:6px;opacity:.7;cursor:pointer}',
				'[data-vsc-layout] [data-vsc-tabx]:hover{opacity:1;background:var(--dsw-alias-interactive-bg-hover)}'
			].join('\n')
			...
			const PANEL_BTN = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 30, minWidth: 30, padding: '0 10px', borderRadius: 8, border: '1px solid var(--dsw-alias-border-l2)', background: 'transparent', color: 'var(--dsw-alias-label-secondary)', fontSize: 13, lineHeight: 1, cursor: 'pointer', transition: 'background var(--ds-transition-duration) var(--ds-ease-in-out), border-color var(--ds-transition-duration) var(--ds-ease-in-out), color var(--ds-transition-duration) var(--ds-ease-in-out)' }
			...
			function startPanelFloatResize(id, e) {
				...
				startDragSession({
					...
					onStep: function (pt) {
						const rect = frame.getBoundingClientRect()
						p.w = clamp(o.w + (pt.x - startX), 320, Math.max(320, rect.width - p.x))
						p.h = clamp(o.h + (pt.y - startY), 200, Math.max(200, rect.height - p.y))
			...
			/* v1.8.1: resizing writes the DOM directly during the drag (like the
			   ...
			   v1.8.8: all of these run through startDragSession (shared listeners,
			   rAF throttle, dragging marker, cleanup registry). */
			function startPluginPanelResize(e) {
				...
				startDragSession({
					...
					onStep: function (x) {
						...
						const w = clamp(base + (startX - x), 240, maxW)
						pp.width = w
			...
			function startListResize(e) {
				...
				startDragSession({
					...
					read: (ev) => ev.clientY,
					onStep: function (y) {
						const h = clamp(base + (y - startY), 44, 260)
						pp.listHeight = h
			...
			function DockPanelBody({ showHeader }) {
				...
				const chips = ids.map(function (id) {
					...
					return React.createElement('button', {
						key: id, type: 'button',
						style: { display: 'inline-flex', alignItems: 'center', gap: 6, height: 30, padding: '0 12px', borderRadius: 8, cursor: 'pointer', fontSize: 14, lineHeight: 1.5, whiteSpace: 'nowrap', flex: 'none', border: '1px solid ' + (on ? 'var(--dsw-alias-border-l2)' : 'var(--dsw-alias-border-l1)'), background: on ? 'var(--dsw-alias-interactive-bg-hover)' : 'transparent', color: on ? 'var(--dsw-alias-label-primary)' : 'var(--dsw-alias-label-secondary)', transition: 'background var(--ds-transition-duration) var(--ds-ease-in-out), border-color var(--ds-transition-duration) var(--ds-ease-in-out), color var(--ds-transition-duration) var(--ds-ease-in-out)' },
				...
				body.push(React.createElement('div', {
					...
					React.createElement('div', {
						style: { flex: 1, minHeight: 0, display: 'flex', alignItems: multiLine ? 'flex-start' : 'center', flexWrap: multiLine ? 'wrap' : 'nowrap', overflowX: multiLine ? 'hidden' : 'auto', overflowY: multiLine ? 'auto' : 'hidden', gap: 6, padding: '8px 12px' }
					},
						...chips,
						React.createElement('button', { key: 'more', type: 'button', style: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 30, minWidth: 30, padding: '0 7px', borderRadius: 8, cursor: 'pointer', fontSize: 14, lineHeight: 1.5, border: '1px solid var(--dsw-alias-border-l1)', background: 'transparent', color: 'var(--dsw-alias-label-secondary)', transition: 'background var(--ds-transition-duration) var(--ds-ease-in-out), border-color var(--ds-transition-duration) var(--ds-ease-in-out), color var(--ds-transition-duration) var(--ds-ease-in-out)' }, title: '全部插件', onClick: function () { setOverflowOpen(!overflowOpen) } }, '···'),
						popover
					),
				...
				if (docked.length > 0) {
					const tabs = docked.map(function (id) {
						...
						return React.createElement('span', {
							key: id,
							style: { display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 8px', fontSize: 14, cursor: 'pointer', borderRadius: 8, color: on ? 'var(--dsw-alias-label-primary)' : 'var(--dsw-alias-label-secondary)', background: on ? 'var(--dsw-alias-interactive-bg-hover)' : 'transparent', border: '1px solid ' + (on ? 'var(--dsw-alias-border-l2)' : 'transparent'), transition: 'background var(--ds-transition-duration) var(--ds-ease-in-out), border-color var(--ds-transition-duration) var(--ds-ease-in-out), color var(--ds-transition-duration) var(--ds-ease-in-out)' },
							onClick: function () { activePanel = id; notify() }
						},
							panelDefs.get(id).title,
							React.createElement('span', { 'data-vsc-tabx': '', title: '关闭', onClick: function (ev) { ev.stopPropagation(); dockApi.closePanel(id) } }, '×'),
							React.createElement('span', { 'data-vsc-tabx': '', title: '浮动', onClick: function (ev) { ev.stopPropagation(); dockApi.floatPanel(id) } }, '⧉')
						)
					})
					body.push(React.createElement('div', { key: 'tabs', style: { display: 'flex', gap: 6, padding: '4px 12px', borderBottom: '1px solid var(--dsw-alias-border-l1)', flexWrap: 'wrap', flex: 'none' } }, ...tabs))
				}
