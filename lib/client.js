		...
		function apply(ctx) {
			...
			function applyDockedResize(kind, p, startX, startY, base, rect) {
				...
				if (side === 'left' || side === 'right') {
					...
					const minW = 120
					/* v1.12.0: width cap raised to frame - CONV_MIN_W — the docked
					   card can grow almost as wide as the user wants while the
					   conversation column keeps >=CONV_MIN_W (v1.12.8: 480). */
					const maxW = Math.max(minW, rect.width - CONV_MIN_W)
			...
			/* v1.12.0: classic three-column panel width drag — updates the
			   ...
			   persists the width on release. Range: 240 .. frame-CONV_MIN_W
			   (the conversation column keeps >=480px, v1.12.8). */
			function startClassicPanelResize(e) {
				...
				startDragSession({
					...
					onStep: function (x) {
						const rect = frame.getBoundingClientRect()
						const maxW = Math.max(240, Math.round(rect.width) - CONV_MIN_W)
