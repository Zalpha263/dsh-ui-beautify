	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SHEET = [
				...
				/* bottom-docked junction (v1.4.7): 1) the FRAME paints the app background so any strip
				   below the conversation column (its shipped height:100% can resolve to auto against a
				   1fr row) shows only same-color background; 2) gap/alignment pinned to 0; 3) the
				   conversation root is forced to fill the column; 4) the panel's border-top is the
				   divider — the junction is always a clean 1px line */
				'[data-vsc-layout][data-vsc-dock-bottom]{background:var(--dsw-alias-bg-base)!important;gap:0!important;align-content:stretch!important}',
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]{background:var(--dsw-alias-bg-base)!important}',
