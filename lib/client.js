	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function saveState() {
				...
				// Quota exceeded (large background data URL). NEVER overwrite the
				// saved background with null here: that would permanently wipe it
				// on every restore round-trip.
				...
				if (old && typeof old.bg === "string" && old.bg !== payload.bg) {
					// Keep the previous background; refresh only the preset field.
					tryPersist({ preset: state.preset, bg: old.bg });
				}
				// If a fresh large image just failed to persist, downscale it and
				// store the smaller copy so it survives a restart.
			...
			function restoreState() {
				...
				// Restore the background FIRST so any saveState() triggered while
				// applying the preset below never overwrites it with null.
			...
			/**
			 * Clear the background image at runtime. With `persist` (default true,
			 * the user clicking 清除) the cleared state is written to storage.
			 * Pass false for teardown/transient clears so an unload or a
			 * re-apply step never wipes a saved background from localStorage.
			 */
			...
			ctx.effect(function cleanup() {
				return function dispose() {
					if (presetTokenDisposer) { presetTokenDisposer(); presetTokenDisposer = null; }
					if (presetCssDisposer) { presetCssDisposer(); presetCssDisposer = null; }
					// Unload must NOT write storage: persisting the cleared bg here
					// would wipe the saved background on every DSH restart/reload.
					clearBackgroundImage(false);
				};
			});
