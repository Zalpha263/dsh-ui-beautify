window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			function oppositeColumnWidth(card) {
				let total = 0
				const dVis = dockVisible()
				for (const k of ['sidebar', 'conversation', 'dock']) {
					if (k === card) continue
					const c = layoutState[k]
					if (c.dock === 'float') continue
					if (k === 'dock' && !dVis) continue
					if (c.dock === 'left' || c.dock === 'right') {
						total += k === 'sidebar' && collapsedSidebar ? SIDEBAR_RAIL : c.width
					}
				}
				return total
			}
			function applyDockedResize(kind, p, startX, startY, base, rect) {
				...
				if (side === 'left' || side === 'right') {
					...
					const minW = 120
					/* v1.12.0: width cap = frame - opposite column - CONV_MIN_W —
					   the docked card can grow while the conversation column keeps
					   >=CONV_MIN_W AND the opposite fixed column keeps its width
					   (v1.12.9 fixed the missing opposite-column term). */
					const maxW = Math.max(minW, rect.width - oppositeColumnWidth(card) - CONV_MIN_W)
