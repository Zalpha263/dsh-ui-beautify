		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.7.0：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			function zoneRect(id, w, h) {
				switch (id) {
					...
					default: return { left: w * 0.35, top: h * 0.3, width: w * 0.3, height: h * 0.4 }
				}
			}
			...
			function applyLayout() {
				...
				const isF = { sb: sbF, cv: cvF, dk: dkF }
				for (const k of ['sb', 'cv', 'dk']) {
					if (isF[k]) continue
					if (k === 'dk' && !dVis) continue
					const pos = layoutState[k].dock
				...
				const rowOf = {}
				const cols = []
				const railW = (k) => (k === 'sb' && collapsedSidebar ? SIDEBAR_RAIL : layoutState[k].width)
				...
				applyEdge(sbEl, 'sb'); applyEdge(cvEl, 'cv'); applyEdge(dkEl, 'dk')

				frame.toggleAttribute('data-vsc-dock-bottom', slots.bottom !== null)

				setVar('--vsc-cols', cols.join(' '))
				setVar('--vsc-rows', rows.join(' '))
				setVar('--vsc-sb-col', cardCol('sb')); setVar('--vsc-sb-row', cardRow('sb'))
				setVar('--vsc-cv-col', cardCol('cv')); setVar('--vsc-cv-row', cardRow('cv'))
				setVar('--vsc-dk-col', cardCol('dk')); setVar('--vsc-dk-row', cardRow('dk'))
			...
			function DockHost() {
				...
				if (docked.length > 0) {
					/* the panel host always follows the dock card's actual grid placement */
					const rect = cardRect('dock')
					if (rect !== null) {
			...
			function SettingsPanel() {
				...
				return React.createElement('div', { style: base },
					...
					React.createElement('div', { style: { marginBottom: 18 } },
						React.createElement('span', { style: labelStyle }, '停靠卡（可停靠任意区域/浮动）'),
						React.createElement('div', { style: rowStyle },
