	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SHEET = [
				...
				'[data-vsc-layout] [data-vsc-tabx]{display:inline-flex;align-items:center;justify-content:center;width:20px;height:20px;border-radius:6px;opacity:.7;cursor:pointer}',
			...
			const PANEL_BTN = { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 4, height: 30, minWidth: 30, padding: '0 10px', borderRadius: 8, border: '1px solid var(--dsw-alias-border-l2)', background: 'transparent', color: 'var(--dsw-alias-label-secondary)', fontSize: 13, lineHeight: 1, cursor: 'pointer', transition: 'background var(--ds-transition-duration) var(--ds-ease-in-out), border-color var(--ds-transition-duration) var(--ds-ease-in-out), color var(--ds-transition-duration) var(--ds-ease-in-out)' }
			...
			function pushFloatChrome(parts) {
				...
				for (const card of ['sidebar', 'conversation', 'dock']) {
					...
					if (card === 'dock') {
						...
						}, React.createElement(CloseIcon, { size: 12 })))
					}
					const dirs = [
				...
				return React.createElement('div', {
					...
					children: [
						React.createElement('div', {
							...
							React.createElement('button', { type: 'button', style: PANEL_BTN, onPointerDown: function (e) { e.stopPropagation() }, onClick: function () { dockApi.closePanel(id) } }, React.createElement(CloseIcon, { size: 14 }))
						),
			...
			function DockPanelBody({ showHeader }) {
				...
				if (showHeader) {
					...
					body.push(React.createElement('div', {
						...
						isClassic && React.createElement('button', { type: 'button', style: PANEL_BTN, onPointerDown: function (e) { e.stopPropagation() }, onClick: function () { layoutState.pluginPanel.float = true; saveLayoutState(); notify() } }, React.createElement(FloatIcon, { size: 14 }), '浮动'),
						React.createElement('button', { type: 'button', style: PANEL_BTN, onPointerDown: function (e) { e.stopPropagation() }, onClick: function () { closeDock() } }, React.createElement(CloseIcon, { size: 14 }))
					))
				}
				...
				if (docked.length > 0) {
					const tabs = docked.map(function (id) {
						...
						return React.createElement('span', {
							...
							React.createElement('span', { 'data-vsc-tabx': '', title: '关闭', onClick: function (ev) { ev.stopPropagation(); dockApi.closePanel(id) } }, React.createElement(CloseIcon, { size: 11 })),
