window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			/* ---------- css ---------- */
			const SHEET = [
				'[data-vsc-layout]{grid-template-columns:var(--vsc-cols,280px minmax(320px,1fr))!important;grid-template-rows:var(--vsc-rows,minmax(0,1fr))!important}',
				...
				'[data-vsc-pp3]{grid-template-columns:var(--vsc-pp-cols,280px minmax(320px,1fr) 400px)!important}',
			...
			/* v1.12.6: the center column (conversation) keeps at least this much,
			   so side cards can never squeeze it away entirely — the column track
			   gets a minmax(CONV_MIN_W,1fr) floor in every layout stamp. */
			const CONV_MIN_W = 320
			...
			function applyClassicLayout() {
				...
				setVar('--vsc-pp-cols', sidebarTrack() + ' minmax(' + CONV_MIN_W + 'px,1fr) ' + layoutState.pluginPanel.width + 'px')
			}
			function applyLayout() {
				...
				/* v1.12.6: the center/free tracks carry a CONV_MIN_W floor — side
				   cards can grow as before (max frame-200), but the conversation
				   column can never be squeezed away entirely. */
				const freeCol = 'minmax(' + CONV_MIN_W + 'px,1fr)'
				...
				else if (slots.left && slots.right) cols.push(freeCol) /* free middle column keeps the right card on the right */
				...
				if (cols.length === 1 && !cols[0].includes('1fr')) cols.push(freeCol)
			...
			function applyDockedResize(kind, p, startX, startY, base, rect) {
				...
				if (side === 'left' || side === 'right') {
					/* v1.12.5: docked min widths are per-card now. The dock card keeps
					   the same floor as its float form (FLOAT_MIN_W.dock); sidebar
					   stays at 120 because the squeeze-to-rail fold depends on <140. */
					const minW = card === 'dock' ? 280 : 120
					...
					/* v1.12.5: docked min heights are per-card too (dock 240 == FLOAT_MIN_H.dock). */
					const minH = card === 'dock' ? 240 : 120
			...
			function startDrag(kind, card, e) {
				...
				startDragSession({
					...
					onUp: function () {
						...
						/* v1.12.5: the dock card's squeeze-to-close gesture is gone —
						   its docked min width (280) never dips below the old 140
						   threshold, so this branch could never fire again. Closing
						   the dock card is done via its titlebar × or the
						   「🧩 插件面板」 toggle instead. */
