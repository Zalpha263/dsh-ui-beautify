		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.7.4：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* composer seat: intentionally left untouched (v1.7.3) — the shipped
				   transparent→bg-base fade is restored so the composer area matches the native
				   webUI; the perceived "unfilled block" comes from scrollbar-gutter:stable on
				   the scrollBody (the 8px gutter is outside the seat), we will design a proper
				   transition for it later */
			].join('\n')
