	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SHEET = [
				...
				/* bottom-docked junction: 1) the conversation column paints the app background so no
				   transparent strip can show; 2) the conversation root is forced to fill the column
				   (its shipped height:100% can resolve to auto against a 1fr row); 3) the panel's
				   border-top is the divider — the junction is always a clean 1px line */
