		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.5.2：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* bottom strip seal (v1.5.2): the shipped ConversationRoot can leave a thin
				   unfilled strip at the column bottom (its height:100% resolves against the
				   stretched grid column), which v1.5.1's removed column paint used to mask.
				   A bottom-anchored ::after only shows where the root does not reach — it sits
				   at z-index:-1 UNDER the root, so it never stacks a second translucent layer
				   over the chat (no re-dimming) and stays invisible when the root fills */
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]:not([data-vsc-float]){position:relative;isolation:isolate}',
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]:not([data-vsc-float])::after{content:"";position:absolute;left:0;right:0;bottom:0;height:8px;background:var(--dsw-alias-bg-base);z-index:-1}',
