		...
		function apply(ctx) {
			...
			function startPluginPanelResize(e) {
				...
				function step() {
					...
					pp.width = clamp(base + (startX - latest), 240, maxW)
					notify()
				}
				function up() {
					window.removeEventListener('pointermove', move)
			...
			function startListResize(e) {
				...
				function step() {
					raf = null
					pp.listHeight = clamp(base + (latest - startY), 36, 260)
					notify()
				}
				function up() {
					window.removeEventListener('pointermove', move)
			...
			function startPluginPanelFloatResize(e) {
				if (frame === null) return
				e.preventDefault()
				e.stopPropagation()
				const box = e.currentTarget.closest('[data-vsc-ppfloat]')
				...
				function step() {
					raf = null
					const rect = frame.getBoundingClientRect()
					pp.fw = clamp(o.w + (latest.x - startX), 320, Math.max(320, rect.width - pp.fx))
					pp.fh = clamp(o.h + (latest.y - startY), 200, Math.max(200, rect.height - pp.fy))
					box.style.width = pp.fw + 'px'
					box.style.height = pp.fh + 'px'
				}
				function up() {
					window.removeEventListener('pointermove', move)
					window.removeEventListener('pointerup', up)
				...
				return React.createElement('div', {
					...
					children: [
						...
						React.createElement('div', {
							key: 'se',
							style: { position: 'absolute', right: 0, bottom: 0, width: 14, height: 14, cursor: 'nwse-resize', zIndex: 2 },
							onPointerDown: function (e) { startPluginPanelFloatResize(e) }
						})
					]
				})
			}
			...
			function DockHost() {
				...
				const pp = layoutState.pluginPanel
				if (isCard) {
					/* card mode: the plugin panel follows the dock card's grid placement */
					...
					parts.push(React.createElement('div', {
						key: 'dhost-classic',
						style: { position: 'absolute', left: left, top: 0, width: w, height: h, zIndex: 21, display: 'flex', flexDirection: 'column', overflow: 'hidden', pointerEvents: 'auto', background: 'color-mix(in srgb, var(--dsw-alias-bg-base) 88%, transparent)', backdropFilter: 'blur(12px)', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: 12, boxSizing: 'border-box' },
						children: [
