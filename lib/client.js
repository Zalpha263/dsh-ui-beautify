		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.6.0：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const DOCK_POSITIONS = {
				sidebar: ['left', 'float'],
				conversation: ['center', 'float'],
				dock: ['right', 'bottom', 'float']
			}
			function mergeLayoutState(saved) {
				...
				if (saved && typeof saved === 'object') {
					...
					const pick = function (src, dst, keys, card) {
						...
						if (typeof src.dock === 'string' && DOCK_POSITIONS[card].includes(src.dock)) dst.dock = src.dock
					}
					pick(saved.sidebar, base.sidebar, ['width', 'fx', 'fy', 'fw', 'fh'], 'sidebar')
					pick(saved.conversation, base.conversation, ['fx', 'fy', 'fw', 'fh'], 'conversation')
					pick(saved.dock, base.dock, ['width', 'height', 'fx', 'fy', 'fw', 'fh'], 'dock')
			...
			function applyLayout() {
				...
				let dkRow = null
				if (dkR) {
					/* keep the dock pinned to the RIGHT edge even when both other cards float:
					   an empty grid must first reserve a free column for the conversation slot,
					   otherwise the dock's single track lands on the left */
					if (cols.length === 0) cols.push('minmax(0,1fr)')
					cols.push(dk.width + 'px')
					dkCol = String(cols.length)
					dkRow = '1 / -1'
				}
				if (dkB) {
			...
			function pushDockedHandles(parts) {
				...
				if (dkR) {
					parts.push(React.createElement('div', {
						...
						style: { position: 'absolute', left: Math.max(0, w - dk.width - 4), top: 0, bottom: 0, width: 8, cursor: 'col-resize', zIndex: 30, touchAction: 'none', pointerEvents: 'auto' },
			...
			function SettingsPanel() {
				...
				return React.createElement('div', { style: base },
					...
					React.createElement('div', { style: { marginBottom: 18 } },
						...
						React.createElement('div', { style: rowStyle },
							React.createElement('button', { type: 'button', style: pill(layoutState.dock.open), onClick: function () { layoutState.dock.open = true; applyLayout(); saveLayoutState() } }, '打开'),
							React.createElement('button', { type: 'button', style: pill(!layoutState.dock.open), onClick: function () { closeDock() } }, '关闭')
						)
					),
