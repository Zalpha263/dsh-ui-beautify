	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const SHEET = [
				...
				/* v1.8.2: the dock card is the positioning anchor for the plugin
				   panel (portaled inside it as an inset:0 layer). Docked cards get
				   position:relative; the float rule below (position:absolute +
				   !important) still wins when the card is floated. */
				'[data-vsc-layout] [data-vsc-card="dock"]{position:relative}',
