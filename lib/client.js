	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const LAYOUT_DEFAULTS = {
				...
				pluginPanel: { open: false, float: false, width: 400, fx: 140, fy: 80, fw: 560, fh: 420, listHeight: 48 },
				panels: {}
			}
			...
			function DockPanelBody({ showHeader }) {
				...
				if (showHeader) {
					...
					body.push(React.createElement('div', {
						key: 'bar',
						style: { display: 'flex', alignItems: 'center', gap: 6, padding: '5px 10px', fontSize: 12, color: 'var(--dsw-alias-label-secondary)', cursor: 'grab', userSelect: 'none', borderBottom: '1px solid var(--dsw-alias-border-l1)', flex: 'none' },
				...
				const chips = ids.map(function (id) {
					...
					return React.createElement('button', {
						key: id, type: 'button',
						style: { display: 'inline-flex', alignItems: 'center', gap: 6, height: 30, padding: '0 12px', borderRadius: 8, cursor: 'pointer', fontSize: 13, lineHeight: 1.5, whiteSpace: 'nowrap', flex: 'none', border: '1px solid ' + (on ? 'var(--dsw-alias-border-l2)' : 'var(--dsw-alias-border-l1)'), background: on ? 'var(--dsw-alias-interactive-bg-hover)' : 'transparent', color: on ? 'var(--dsw-alias-label-primary)' : 'var(--dsw-alias-label-secondary)' },
						...
						React.createElement('span', { style: { fontSize: 10, opacity: .8 } }, on ? '●' : '○')
					)
				})
				const popover = overflowOpen ? React.createElement('div', {
					style: { position: 'absolute', right: 6, top: 42, zIndex: 5, minWidth: 220, maxHeight: 280, overflowY: 'auto', padding: 6, display: 'flex', flexDirection: 'column', gap: 2, background: 'var(--dsw-alias-bg-layer-1)', border: '1px solid var(--dsw-alias-border-l2)', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,.18)' }
				},
				...
				body.push(React.createElement('div', {
					...
					React.createElement('div', {
						...
						...chips,
						React.createElement('button', { key: 'more', type: 'button', style: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', height: 30, minWidth: 30, padding: '0 7px', borderRadius: 8, cursor: 'pointer', fontSize: 13, lineHeight: 1.5, border: '1px solid var(--dsw-alias-border-l1)', background: 'transparent', color: 'var(--dsw-alias-label-secondary)' }, title: '全部插件', onClick: function () { setOverflowOpen(!overflowOpen) } }, '···'),
						popover
					),
