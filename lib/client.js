		...
		function apply(ctx) {
			...
			function applyDockedResize(kind, p, startX, startY, base, rect) {
				...
				if (side === 'left' || side === 'right') {
					/* v1.12.7: docked min width back to the v1.12.4 floor (120) for
					   every card — the v1.12.5 per-card raise (dock 280) blocked
					   squeezing the plugin panel, which users rely on. The center
					   column floor (CONV_MIN_W, v1.12.6) still protects the
					   conversation column from being squeezed away. */
					const minW = 120
