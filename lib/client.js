		...
		function apply(ctx) {
			...
			function Panel() {
				...
				return React.createElement("div", { style: base },
					...
					React.createElement("p", { style: p }, "v1.5.0：设置自动记忆，全局强调色（文件夹、加载螺旋、标签高亮、Deep Diving、超链接等）跟随预设。"),
			...
			const SHEET = [
				...
				/* bottom-docked junction (v1.4.9): 1) row 1 is a deterministic calc in applyLayout,
				   so the column fills exactly; 2) the dock card paints bg-base in every docked
				   state (longhand, does not touch the frame's background-image) — the details
				   slot is scope="session" and renders NOTHING while no session is active, which
				   used to leave the dock column transparent and exposed the old 8px box-shadow
				   band as a hard, uncoordinated stripe between the bars; 3) gap and alignment
				   pinned to 0; 4) the panel's border-top stays the single divider */
				'[data-vsc-layout][data-vsc-dock-bottom]{gap:0!important;align-content:stretch!important}',
				'[data-vsc-layout][data-vsc-dock-bottom] [data-vsc-card="conversation"]:not([data-vsc-float]){background-color:var(--dsw-alias-bg-base)!important}',
				'[data-vsc-layout][data-vsc-dock-bottom] [data-slot="conversation"] > *{flex:1!important;min-height:0!important}',
				/* dock card surface: always opaque when docked (the shipped DetailsPanel paints
				   bg-base itself, so this only shows when its content is absent) */
				'[data-vsc-layout] [data-vsc-card="dock"]:not([data-vsc-float]){background-color:var(--dsw-alias-bg-base)!important}',
