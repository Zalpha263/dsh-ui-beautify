window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			const PANEL_BTN = { border: '1px solid var(--dsw-alias-border-l2)', background: 'transparent', color: 'var(--dsw-alias-label-secondary)', borderRadius: 8, fontSize: 12, padding: '3px 8px', cursor: 'pointer', lineHeight: 1.5, transition: 'background var(--ds-transition-duration) var(--ds-ease-in-out), border-color var(--ds-transition-duration) var(--ds-ease-in-out), color var(--ds-transition-duration) var(--ds-ease-in-out)' }
			...
			function DockPanelBody({ showHeader }) {
				...
				const chips = ids.map(function (id) {
					...
					return React.createElement('button', {
						...
						React.createElement('span', { style: { fontSize: 14 } }, def.icon || '🧩'),
				...
				body.push(React.createElement('div', {
					...
					React.createElement('div', {
						style: { flex: 1, minHeight: 0, display: 'flex', alignItems: multiLine ? 'flex-start' : 'center', flexWrap: multiLine ? 'wrap' : 'nowrap', overflowX: multiLine ? 'hidden' : 'auto', overflowY: multiLine ? 'auto' : 'hidden', gap: 4, padding: '8px 12px' }
					},
						...chips,
