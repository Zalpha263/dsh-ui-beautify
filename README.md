...
- **v1.5.3**：统一卡片描边体系（替代 v1.5.2 被撤回的色带方案）——几何探针确认吸附底部时对话列/停靠卡/输入区接缝均为 0px（无缝隙），此前"看起来像缝隙 / 没有分隔线"源于描边不统一：右停靠时分隔线是停靠卡与详情面板两条 `border-left` 叠加，底部时只剩一条深色下几乎不可见的 `border-top`。现统一为：**相邻卡片之间恒为一条 1px `border-l2` 分隔线，统一由对话卡承担**（右停靠→右缘线，底部→底缘线）；停靠卡与详情面板不再绘制接缝描边；侧栏右缘（卡片模式下）统一为 `border-l2`。整体配色 / 圆角 / 透明度风格不变，无新增底色与阴影带。
