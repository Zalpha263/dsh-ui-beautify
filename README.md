...
- **v1.5.2**：修复「吸附底部时中间栏与底栏之间出现未填充缝隙、分隔线缺失」——DSH 的对话根节点在拉伸网格列下可能留下一薄层列底条带，v1.5.1 移除对话列铺色后该条带暴露（露出整页背景）。现给对话列加**贴底 `::after` 密封带**（`z-index:-1` 藏在根节点之下，只在根节点未覆盖处显现，不叠加第二层半透明、不再次变暗）；底栏顶部的 1px `border-l2` 分隔线随之恢复为接缝处唯一分界线。
