...
- **v1.7.4**：对话卡**停靠到非中心区域**（上/下/左/右）时，表面向侧栏色靠 25% 形成轻微色差（`color-mix(sidebar-fill 25%, bg-base)`），对话内容背景改为透出该底色——停靠态的接缝观感与其它卡片（侧栏色差 + `l3` 线）一致；中心停靠与浮动形态保持原样。
- **v1.7.3**：输入框区域**恢复原生 webUI 样式**——撤除对 composer seat 背景的自定义（回归 DSH 原版 `transparent→bg-base` 淡入）。「色块疑似没填满」的成因确认为 `scrollbar-gutter:stable` 在滚动区右侧预留的 8px 槽（gutter 位于 seat 背景覆盖范围之外），过渡方案待定（见后续版本）。
- **v1.7.2**：①吸附提示再缩小约 20%（边缘区 22%→18%，中间区同步缩小）；②接缝线修复——中心槽卡也统一清除 DSH 原生边框（侧栏/停靠卡停靠中间时不再残留原生 `border-right`/`border-left` 造成双线/孤线），任意区域组合的接缝线一致；单卡停靠（其余浮动）时自动补自由列，卡片不再孤悬在狭窄轨道里；③输入框区域改为与对话卡**同色实底**（`bg-base`）——横向填满、深浅色统一、无渐变带与色块，消息滚动到输入框后自然隐藏（如需消息隐约透出可改半透明）。
- **v1.7.1**：修复 v1.7.0 的崩溃——五区域槽求解器误用缩写键（`sb/cv/dk`）索引 `layoutState`（真实键为 `sidebar/conversation/dock`），导致 `applyLayout` 抛 TypeError：卡片无法拖出、吸附后无法缩放、中间栏下方出现大片空白。槽位、边缘（`data-vsc-edge`）与 grid 变量统一改用全名。
- **v1.7.0**：**五区域通用吸附**——上/下/左/中/右五个吸附区域对**所有卡片**开放（原侧栏只能停左、对话只能居中、停靠卡只能右/底）；同一区域只能容纳一张卡，后吸附的卡会把原卡挤成浮动；顶/底槽卡片横跨全宽，左/中/右卡片共享主行；分隔线改为**边缘驱动**（`data-vsc-edge`，任意卡片停靠任意区域都自动在邻接侧画一条 `border-l3` 线）；停靠尺寸（宽/高）每卡独立记忆，手柄按槽位生成（左/右槽拖宽、顶/底槽拖高）。**吸附提示优化**——区域缩小到边缘 22%，且**只有鼠标拖到该区域时才显示**单个高亮提示；面板宿主（DockHost）改为直接跟随停靠卡的实际网格位置。
- **v1.6.0**：代码审查与健壮性修复——①设置页「停靠卡」的打开/关闭按钮增加选中高亮（与布局模式等其它设置按钮统一）；②持久化状态中的停靠位置值改为**白名单校验**（侧栏 left/float、对话 center/float、停靠卡 right/bottom/float），损坏/旧版 localStorage 不再导致卡片落到未定义状态；③修复「侧栏与对话都浮动时停靠卡右停靠错位到左侧」——grid 为空时先补一列自由列再放停靠列；④frame 子节点被 React 整体替换时自动重新标记并应用布局（此前只在 frame 被移除时重建）；⑤`removeAll` 补清理 `data-vsc-dragging`；⑥窄窗口下右列缩放手柄 left 下限归零。
- **v1.5.6**：停靠卡改用**侧边栏同款底色**（`--dsw-specific-sidebar-fill`）——与对话栏（`bg-base`）形成与「侧栏↔对话栏」完全相同的**背景色差**，接缝不再依赖线的可见度。详情面板在卡片模式下强制透明、空详情占位提示不再自带底色（保持单层，无半透明叠加变暗问题）；`border-l3` 分隔线保留。浮动态停靠卡仍为毛玻璃样式。
- **v1.5.5**：接缝线提亮——侧栏右缘与停靠卡接缝（左缘/顶缘）统一改用 `border-l3`（深色 0.16 白 / 浅色 0.12 黑，DSH 原生强分隔档）。此前用 `border-l2` 单线（0.12/0.10）在相同底色（对话栏与停靠卡均为 `bg-base`）上肉眼不可见——侧栏接缝之所以"有颜色"是 `sidebar-fill` 与 `bg-base` 的**背景色差**，并非那条线。两侧分隔线现为同一 token、同一宽度，清晰可见。
- **v1.5.4**：接缝线改由**停靠卡**直接绘制（右停靠→左缘线，底部→顶缘线），颜色统一为 `border-l2`（与侧栏右缘同色）。回退 v1.5.3「对话卡承担接缝线」的方案：content-box 模型下对话卡一旦缺线会把内容压缩 1px 露出透明缝，且该方案依赖新增 frame 属性，链路脆弱。
...
- **v1.5.1**：修复「右侧栏颜色太深」「吸附底部时对话卡不透明度变化」——两者同源：背景图模式下 `bg-base` 是 85% 半透明色，而 v1.4.6/1.4.9 在**卡片元素上额外铺了同色底色**，与卡片内部（对话根节点 / 详情面板）自带的 `bg-base` 叠加成两层 → 停靠卡比对话栏深、吸附底部时对话栏整体变暗。现改为：①对话列不再铺色（出厂根节点底色足够，行高 `calc(100% - <高>px)` 保证接缝精确）；②停靠卡不再铺色（详情面板自带底色）；③「无活动会话」占位提示自带单层 `bg-base` 底色，空详情态与其它卡片观感一致。
...
- **v1.4.9**：修复两个底部/停靠卡观感 bug——
  ...
  2. **归档会话后右侧停靠栏被「清空」**：DSH 的 `details` 槽是 `scope:"session"`——没有活动会话时该槽渲染为空，停靠卡因此变成一块透明空白（看起来像被清空）。现为空的详情槽渲染**占位提示**（「无活动会话」居中弱化文字），停靠卡保持完整面板外观，归档 / 新建会话不再出现大变化。
- **v1.4.8**：修复 v1.4.7 的回归——底部吸附时整页背景图消失。根因：v1.4.7 在 frame 上用了 `background` **简写**属性，把同一元素上的 `background-image`（背景图绘制点）重置为 none。现改为：①frame 不再绘制背景；②对话列改用 `background-color` 长写（不碰背景图）+ 列下方 8px 同色阴影带兜底（无缝隙时不可见，被面板盖住）；③兜底规则加 `:not([data-vsc-float])`，避免误伤浮动态对话卡。底部吸附时背景图恢复正常显示。
