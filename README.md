...
- **v1.7.0**：**五区域通用吸附**——上/下/左/中/右五个吸附区域对**所有卡片**开放（原侧栏只能停左、对话只能居中、停靠卡只能右/底）；同一区域只能容纳一张卡，后吸附的卡会把原卡挤成浮动；顶/底槽卡片横跨全宽，左/中/右卡片共享主行；分隔线改为**边缘驱动**（`data-vsc-edge`，任意卡片停靠任意区域都自动在邻接侧画一条 `border-l3` 线）；停靠尺寸（宽/高）每卡独立记忆，手柄按槽位生成（左/右槽拖宽、顶/底槽拖高）。**吸附提示优化**——区域缩小到边缘 22%，且**只有鼠标拖到该区域时才显示**单个高亮提示（不再一拖就铺满屏提示）；面板宿主（DockHost）改为直接跟随停靠卡的实际网格位置。
- **v1.6.0**：代码审查与健壮性修复——①设置页「停靠卡」的打开/关闭按钮增加选中高亮（与布局模式等其它设置按钮统一）；②持久化状态中的停靠位置值改为**白名单校验**；③修复「侧栏与对话都浮动时停靠卡右停靠错位到左侧」；④frame 子节点被 React 整体替换时自动重新标记并应用布局；⑤`removeAll` 补清理 `data-vsc-dragging`；⑥窄窗口下右列缩放手柄 left 下限归零。
